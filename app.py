...
class ClaimRequest(BaseModel):
    claim_id:               str   = Field(...,  json_schema_extra={"example": "CLM_000001"})
    insured_id:             str   = Field(...,  json_schema_extra={"example": "INS_012345"})
    incident_type:          str   = Field(...,  json_schema_extra={"example": "Collision simple"})
    incident_severity:      str   = Field(...,  json_schema_extra={"example": "important"})
    incident_date:          str   = Field(...,  json_schema_extra={"example": "2024-08-15"})
    incident_hour:          int   = Field(12,   ge=0, le=23)
    incident_day_of_week:   int   = Field(0,    ge=0, le=6)
    declaration_delay_days: int   = Field(1,    ge=0)
    region:                 str   = Field(...,  json_schema_extra={"example": "Île-de-France"})
    vehicle_brand:          str   = Field(...,  json_schema_extra={"example": "BMW"})
    vehicle_age:            int   = Field(2,    ge=0, le=30)
    total_claim_amount:     float = Field(...,  gt=0, json_schema_extra={"example": 19800.0})
    driver_age:             int   = Field(30,   ge=18, le=90)
    months_as_customer:     int   = Field(24,   ge=1)
    annual_income:          float = Field(...,  gt=0, json_schema_extra={"example": 28000.0})
    profession:             str   = Field(...,  json_schema_extra={"example": "commercial"})
    ...
    incident_description:   str   = Field("",   json_schema_extra={"example": "Collision sur parking."})
